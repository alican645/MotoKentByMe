...
class _GroupSettingViewState extends State<GroupSettingView> {
  ...
  Future<void> _reportUserOrGroup(
    ...
    var selectedID =
        await ComplaintDialog.show(context: context, reasons: list,title: "Grubu Şikayet Nedenini");
  ...
  Future<void> _viewProfile(UserModel2 user, BuildContext context, isMe) async {
    if (isMe) {
  ...
  Future<void> _sendMessage(UserModel2 user, BuildContext context) async {
    try {
      var response = await context
          .read<GroupSettingViewmodel>()
