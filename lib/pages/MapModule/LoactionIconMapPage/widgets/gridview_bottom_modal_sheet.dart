...
class IconPickerModal extends StatefulWidget {
  final Function(int id, String iconPath, BuildContext context) onIconSelected;
...
class _IconPickerModalState extends State<IconPickerModal> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback(
      (timeStamp) async {
        await context.read<LoactionIconMapViewmodel>().fetchCustomMarkerItem();
      },
    );
  }
  ...
  Widget build(BuildContext context) {
    return Consumer<LoactionIconMapViewmodel>(
      builder: (context, value, child) {
        if (value.fetchCustomMarkerItemFlag == false) {
          ...
          itemBuilder: (context, index) {
            if (index == 0) {
              return IconButton(
                onPressed: () {
                  context.push(
                    '${AppRoutes.mapPage}/${AppRoutes.buyCoinPage}',
                  );
                },
                icon: Icon(
