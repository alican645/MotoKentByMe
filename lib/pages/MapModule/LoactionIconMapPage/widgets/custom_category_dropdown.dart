...
class CustomMarkerDropdown extends StatefulWidget {
  ...
  @override
  State<CustomMarkerDropdown> createState() => _CustomMarkerDropdownState();
}

class _CustomMarkerDropdownState extends State<CustomMarkerDropdown> {
  late List<CustomMarkerModel> _filteredMarkers;
  ...
  Widget build(BuildContext context) {
    return DropdownButtonHideUnderline(
      child: DropdownButton2<CustomMarkerModel>(
        ...
        items: _filteredMarkers.map((marker) {
          return DropdownMenuItem<CustomMarkerModel>(
            value: marker,
            child: _buildMarkerItem(marker),
          );
        }).toList(),
        onChanged: (value) {
          if (value != null && widget.onMarkerSelected != null) {
            widget.onMarkerSelected!(value);
          }
        },
        dropdownStyleData: widget.dropdownStyle ?? _defaultDropdownStyle(),
        menuItemStyleData: widget.menuItemStyle ?? _defaultMenuItemStyle(),
      ),
    );
  }

  Widget _buildMarkerItem(CustomMarkerModel marker) {
    return Padding(
  ...
  DropdownStyleData _defaultDropdownStyle() {
  ...
  MenuItemStyleData _defaultMenuItemStyle() {
